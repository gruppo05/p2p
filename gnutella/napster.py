	...
	def start(self):
		...
		while True:
			...
			try:
				...
				if command == "LOGI":
					try:
						IPP2P = connection.recv(55).decode("ascii")
						IPP = connection.recv(5).decode("ascii")
						...
						connection.sendall(("ALGI"+SessionID).encode("utf-8"))
						print("Invio il SessionID --> \033[33m"+SessionID+"\033[m")
						
				elif command == "LOGO":
					
					SessionID = connection.recv(16).decode("ascii")
					...
					connection.sendall(("ALGO"+delete).encode("utf-8"))
					
				elif command == "DELF":
					SessionID = connection.recv(16).decode("ascii")
					Filemd5 = connection.recv(32).decode("ascii")
					...
					connection.sendall(("ADEL"+str(copy)).encode("utf-8"))
					...
					SessionID = connection.recv(16).decode("ascii")
					Ricerca = connection.recv(20).decode("ascii")
					...
					SessionID = connection.recv(16).decode("ascii")
					Filemd5 = connection.recv(32).decode("ascii")
					Filename = connection.recv(100).decode("ascii")
					print(SessionID)
					print(Filemd5)
					print(Filename)
					...
					copy = self.dbReader.fetchone()
					copy = setCopy(copy)
					connection.sendall(("AADD"+copy).encode("utf-8"))
					...
					connection.sendall(("ADRE"+download).encode("utf-8"))
